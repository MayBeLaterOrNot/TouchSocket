...
public sealed class Container : IContainer
{
    ...
    private object ResolveFromEntry(ServiceEntry entry, IResolver resolver, Type[] genericArguments = null)
    {
        ...
        {
            return descriptor.ImplementationFactory(resolver);
        }
        ...
        if (descriptor.Lifetime == Lifetime.Singleton)
        {
            ...
            lock (entry.Lock)
            {
                ...
                var instance = this.CreateInstance(descriptor.ToType, resolver, genericArguments);
        ...
        // 处理瞬态和作用域（在Container级别，作用域和瞬态表现相同）
        var newInstance = this.CreateInstance(descriptor.ToType, resolver, genericArguments);
    ...
    private sealed class ScopedResolver : DisposableObject, IScopedResolver
    {
        ...
        private object ResolveFromScopedEntry(ServiceEntry entry, ServiceKey serviceKey, Type[] genericArguments = null)
        {
            ...
            if (descriptor.ImplementationFactory != null)
            {
                var instance = descriptor.ImplementationFactory(this.Resolver);

                // 如果是Scoped生命周期，缓存实例
                if (descriptor.Lifetime == Lifetime.Scoped)
                {
                    this.m_scopedInstances.TryAdd(serviceKey, instance);
                }
            ...
            // 作用域生命周期：检查是否已在当前作用域创建
            if (descriptor.Lifetime == Lifetime.Scoped)
            {
                return this.m_scopedInstances.GetOrAdd(serviceKey, _ =>
                {
                    var instance = this.m_rootContainer.CreateInstance(descriptor.ToType, this.Resolver, genericArguments);
                    descriptor.OnResolved?.Invoke(instance);
                    return instance;
                });
            }
            ...
            // 瞬态生命周期：每次都创建新实例
            var newInstance = this.m_rootContainer.CreateInstance(descriptor.ToType, this.Resolver, genericArguments);
            descriptor.OnResolved?.Invoke(newInstance);
            return newInstance;
        }
